...
class WxPayApi
{
	...
	public static function notify($callback, &$msg)
	{
		...
		$x_arr =  json_decode(json_encode(simplexml_load_string($xml,'SimpleXMLElement',LIBXML_NOCDATA)) ,true) ;
		
		
		
		$transaction_id = $x_arr['transaction_id'];
		
      
		$out_trade_no = trim('',$x_arr['out_trade_no']);
		
		// 修改订单的状态
		// $db="mysql:host=localhost;dbname=leifeng";
		// $user="leifeng";
		// $pas="root123";
		// $pdo =new PDO($db,$user,$pas);

		// $id = 248;
		// $affected_rows = $pdo->exec("UPDATE os_order SET pay_status=1 ,type=2 where code = ".$out_trade_no);
	...
	{		
		...
		curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,FALSE);  //modify started by liu
		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,FALSE);//严格校验    //modify end
		//设置header
