...
$input->SetBody("test");	//商品描述，必填
$input->SetAttach("test");	//附加数据
$out_trade_no = WxPayConfig::MCHID.date("YmdHis");
echo $out_trade_no;
$input->SetOut_trade_no($out_trade_no);	//商户订单号，必填

$input->SetTotal_fee("1");	//总金额，必填
$input->SetTime_start(date("YmdHis"));	//交易起始时间
$input->SetTime_expire(date("YmdHis", time() + 600));	//交易结束时间
$input->SetGoods_tag("test");	//商品标记
$input->SetNotify_url("http://paysdk.weixin.qq.com/example/notify.php");	//通知地址，必填
...
<body>
	...
	<script>
        ...
        function check(){
            var url = "http://localhost/WxpayAPI_php_v3/example/orderquery2.php";
