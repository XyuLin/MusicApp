...
class ReadHtmlCacheBehavior {
    ...
    public function run(&$params){
        ...
        if(IS_GET && C('HTML_CACHE_ON'))  {
            ...
            //如果没有缓存 或者已经过期了 就往后执行 也就是这里  那么为了防止雪崩事件 在这里加个文件锁
            //（无论在缓存过期 取数据的一瞬间进来多少人 都只让一个人进去去取信息 读取完后就会生成静态页面 在放开锁）
            global $fp;
            $fp = fopen('.././index_html_cache.lock','r');              //只读模式
            flock($fp,LOCK_EX);                                         //排它锁
            //（所以这样就可以了 只有一个能拿到钥匙过去了  过去就能获取信息  然后系统会自动生成缓存
            //  当有缓存的时候 这里的代码就不会执行这个锁文件了）
            //当第二个人来的时候 就需要判断有没有静态缓存就好了 就可以回到上面代码了 所以再把上面代码拿过来判断就可以了 才能线性执行
            if( false !== $cacheTime && $this->checkHTMLCache(HTML_FILE_NAME,$cacheTime)) { //静态页面有效
                // 读取静态页面输出
                echo Storage::read(HTML_FILE_NAME,'html');
                exit();
